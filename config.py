...
class Gui(QDialog):
    """Class for the GUI control panel"""
    ...
    def remake_preset_generator(self):
        ...
        self.new_preset_name_editor.textChanged.connect(
            lambda: self.check_new_config_name()
        )
    ...
    def remake_preset_chooser(self, master_config_path=MASTER_CONFIG_PATH):
        ...
        self.selectBtn.clicked.connect(
            lambda: self.save_master_dict(self.master_config_dict, master_config_path)
        )
    ...
    def create_dict_editor(self, preset_path):
        ...
        self.saveBtn.clicked.connect(
            lambda: self.save_config_dict(self.config_dict, preset_path)
        )
    ...
    def check_dict(self):
        ...
        try:
            ...
            for key, value in self.config_dict.items():
                ...
                if type(value) == list and key not in [
                    "natures",
                    "synths",
                    "manual_optimum",
                ]:
    ...
    def save_config_dict(self, config_dict, config_path):
        ...
        for file in files:
            if (
                file.split(os.sep)[-1].split(".")[0].upper()
                not in config_dict["natures"]
            ):
...
def get_available_samples():
    ...
    master_config_dict["n_available_samples"] = [
        len(x) for x in available_samples.values()
    ]
