"""
File that contains functions to be used in evolution.py
"""
...
def age2fit(age, a=4):
    """Converts the age to a fitness value on an offset quadratic function"""

    fitness = ((age - a) ** 2 - a ** 2) / a ** 2
...
def compute_optimum(playing, phen_cols, method="mean"):
    ...
    natures = playing.loc[
        :, "nature"
    ]  # save the instrument types for the instr_optima method
...
def symmetry(ind_order, playing):
    """Evaluation function for order symmetry"""
...
def scramble_all(preset_path, subset=["order"]):
    ...
    new = scramble_playing(
        pd.read_csv(f"{preset_path}current/playing.csv"), subset
